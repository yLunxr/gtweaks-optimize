...
if ((Test-Admin) -eq $false)  {
    if ($elevated) {
		...
		$msgTitle = "gtweaks optimizer script. Erro de permissão"
		$msgButton = 'OK'
		$msgImage = 'Error'
...
# ===================================================================
# ================ INÍCIO DA VERIFICAÇÃO DE ATIVAÇÃO ================
# ===================================================================

$ACTIVATION_URL = "https://script.google.com/macros/s/AKfycbxtuVr8HXUXlZsKLP5zYO0A6LdONKATX-lW9z4F1psEDBq-T9A-XGaQcsgZvXCNgyrh/exec"

# Limpa a tela e solicita o token ao usuário
Clear-Host
$token = Read-Host -Prompt "Por favor, insira seu token de ativação para usar o gtweaks optimizer"

# Verifica se o usuário inseriu algo
if ([string]::IsNullOrWhiteSpace($token)) {
    Write-Host "Nenhum token fornecido. O script será encerrado." -ForegroundColor Red
    Start-Sleep -Seconds 5
    exit
}

# Constrói a URL de validação completa com o token
$validationUrl = "$ACTIVATION_URL`?token=$token"

Write-Host "Validando o token, por favor aguarde..." -ForegroundColor Yellow

try {
    # Faz a requisição web para a URL do Google Script
    # O parâmetro -UseBasicParsing evita problemas de dependência com o motor do Internet Explorer
    $response = Invoke-RestMethod -Uri $validationUrl -Method Get -UseBasicParsing

    # Verifica a resposta do servidor.
    # Esta condição assume que uma resposta de SUCESSO contém a palavra "success".
    # Você pode precisar ajustar a palavra ("success", "válido", "ativo") conforme o que seu Google Script retorna.
    if ($response -match "SUCESSO") {
        Write-Host "Token validado com sucesso! Iniciando o script..." -ForegroundColor Green
        Start-Sleep -Seconds 3
    } else {
        Write-Host "Token inválido ou expirado. O script não pode continuar." -ForegroundColor Red
        Write-Host "Resposta do servidor: $response" -ForegroundColor Cyan
        Start-Sleep -Seconds 5
        exit
    }
}
    ...
    Write-Host "Ocorreu um erro ao tentar validar o token." -ForegroundColor Red
    Write-Host "Verifique sua conexão com a internet e tente novamente." -ForegroundColor Red
...
# ===================================================================
# =================== FIM DA VERIFICAÇÃO DE ATIVAÇÃO =================
# ===================================================================
...
$form.Font = $Font; 
$base64IconString = ""
...
$aboutFormNameLabel.Text = '  gtweaks optimizer script'; 
$aboutForm.Controls.Add($aboutFormNameLabel); 
$aboutFormText.Location = '100, 30'; 
$aboutFormText.Size = '300, 20'; $aboutFormText.Text = '         Guilherme Menezes'; 
$aboutForm.Controls.Add($aboutFormText); 
$aboutFormText2.Location = '100, 50'; 
$aboutFormText2.Size = '300, 20';  
$aboutFormText2.Text = '      GitHub.com/yLunxr'; 
...
#MsgBox information
Add-Type -AssemblyName PresentationCore,PresentationFramework
$msgBody = "Todos os comandos foram aplicados, é recomendado reniciar o seu computador. obrigado por usar o gtweaks, windows optimizer!."
$msgTitle = $versionRAW
$msgButton = 'OK'
